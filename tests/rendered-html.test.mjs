...
test("uses the complete photographic set as one three-part narrative", async () => {
...
test("loads honest Open-Meteo states without embedded observations", async () => {
  ...
  assert.match(conditionsSource, /Источник наблюдений/);
  assert.doesNotMatch(pageSource, /\+23°|\+28°|16 августа, 23:15/);
  assert.equal((html.match(/Загрузка/g) ?? []).length, 3);
});

test("locks the weather rail to the approved MATERIAL / 01 token", async () => {
  ...
  assert.equal(
    (conditionsSource.match(/className="condition service-island material-glass"/g) ?? []).length,
    1,
  );
